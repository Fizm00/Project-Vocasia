    ...
    <div className="bg-white py-16 px-4">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-2xl font-extrabold text-[#193F3D] mb-6 text-left transition-all duration-300 hover:translate-x-3">
          Promo Kilat Kost di <span className="text-[#78825B]">Semua Kota!</span>
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {kosts.map((kost) => (
            <div
              ...
              onClick={() => handleCardClick(kost.id)}
            >
              <img
                src={kost.image}
                alt={kost.name}
              ...
              <h3 className="text-base font-extrabold text-[#193F3D] mb-1">{kost.name}</h3>
              <p className="text-xs text-[#193F3D] mb-3">{kost.location}</p>
